...
#
#root.mainloop()
"""Название событие передается в угловых скобках, например, "<Enter>" или "<Leave>". Для события Enter (получение фокуса) определен обработчик-функция entered, которая изменяет текст кнопки:"""
#def entered(event):
#    btn["text"] ="Entered"
...
"""Событие потери фокуса связывается с функцией left:"""
#btn.bind("<Leave>", left)
...
#
#root.mainloop()
"""В данном случае для кнопок для обработки двойного нажатия устанавливается обработчик - функция clicked. Причем события привязывается к кнопкам из пакета tkinter.ttk, поэтому в качестве типа виджетов используется "TButton" (а не просто Button)."""
"""Удаление события"""
